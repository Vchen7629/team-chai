...
def check_distributions(df: pl.DataFrame) -> None:
    ...
    print(f"Activity level distribution: {df.group_by("activity_level").len().sort("activity_level")}")

    returning = returning.with_columns([
        pl.col("activity_level").cast(pl.String).replace(activity_map),
        pl.col("step_goal").cast(pl.Float64),
    ])
    print(f"\nStep goal by activity level (returning users): {returning.group_by("activity_level").agg(
