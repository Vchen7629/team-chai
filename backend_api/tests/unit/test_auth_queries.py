...
@pytest.mark.asyncio
async def test_get_hashed_password_raises_when_user_not_found() -> None:
    session = make_mock_session(None)
    with pytest.raises(ValueError, match="No user account found"):
        await get_hashed_password(session, "unknown")


@pytest.mark.asyncio
async def test_insert_new_user_account_raises_when_username_taken() -> None:
    session = make_mock_session(None)
    with pytest.raises(ValueError, match="Username already taken"):
        await insert_new_user_account(session, make_user_data())
...
async def test_insert_new_user_account_height_conversion(
    ...
    await insert_new_user_account(session, user_data)
